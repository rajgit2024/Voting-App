  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-grow relative">
        {/* Animated Background */}
        <div className="fixed inset-0 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 overflow-hidden -z-10">
        ...
        <main className="container mx-auto px-4 pt-24 pb-16">
          {/* Hero */}
          <motion.section
            className="text-center text-white mt-8 mb-16"
            variants={containerVariants}
            initial="hidden"
            animate={isLoaded ? "visible" : "hidden"}
          >
            <motion.h2 className="text-4xl md:text-6xl font-extrabold drop-shadow-lg" variants={itemVariants}>
            ...
            <motion.div variants={itemVariants} className="mt-8">
              <motion.div className="inline-block relative overflow-hidden" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Link to="/register">
          ...
          >
            {images.map((img, index) => (
              <motion.div
                key={index}
                className="relative w-full sm:w-64 h-80 p-4 bg-white bg-opacity-20 backdrop-blur-md rounded-xl shadow-xl cursor-pointer overflow-hidden"
          ...
          {/* Features */}
          <motion.section
            className="py-12 px-3 bg-white bg-opacity-20 backdrop-blur-md rounded-xl text-center mb-16"
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { title: "Secure Voting", description: "Your vote is encrypted and protected with advanced security protocols.", icon: "🔒" },
                { title: "Easy to Use", description: "User-friendly interface for hassle-free voting experience.", icon: "👍" },
                { title: "Real-Time Results", description: "Get instant access to voting results as they are counted.", icon: "📊" },
              ].map((feature, index) => (
                <motion.div
                  key={index}
          ...
          {/* Call to Action */}
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true, amount: 0.8 }}>
            <motion.h3 className="text-2xl font-bold text-white mb-6" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: 0.2 }} viewport={{ once: true }}>
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Link to="/register">
                <button className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-8 py-4 rounded-lg font-bold shadow-lg hover:shadow-xl transition-all duration-300 relative overflow-hidden group">
                  ...
                  <motion.span className="absolute inset-0 bg-gradient-to-r from-indigo-600 to-purple-600" initial={{ x: "100%" }} whileHover={{ x: 0 }} transition={{ duration: 0.4 }} />
                </button>
              </Link>
            </motion.div>
          </motion.div>
        ...
        {selectedImage && (
          ...
          >
            ...
            >
              ...
              <div className="w-full h-[300px] rounded-md mb-4 bg-cover bg-center" style={{ backgroundImage: `url(${selectedImage.src})` }} />
              <motion.p className="text-gray-600 mb-4" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}>
                {selectedImage.info}
              </motion.p>
              <motion.button
                onClick={() => setSelectedImage(null)}
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="flex justify-center mt-4 space-x-4">
            {["Facebook", "Twitter", "Instagram"].map((social, index) => (
              <motion.a key={index} href="#" className="text-gray-400 hover:text-white transition-colors duration-300" whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.9 }}>
                {social}
              </motion.a>
            ))}
          </div>
